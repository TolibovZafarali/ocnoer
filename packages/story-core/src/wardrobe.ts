import type { CharacterDefinition, RuntimeCharacter } from "./types";
import { isPrimaryLeftStageCharacterSlug } from "./staging";
...
export function resolveDressImagePath(input: {
  ...
  const baseEmotion =
    input.character.emotions.find(
      (emotion) => emotion.key === input.emotionKey
    ) ?? null;
